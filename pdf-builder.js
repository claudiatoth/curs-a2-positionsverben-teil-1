...
(function () {
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += mcBlock('Übung 1 — Wo? sau Wohin? (poziție statică vs. mișcare)', 'Wo? = poziție STATICĂ (verbe: liegen/stehen/sitzen/hängen/stecken) → Dativ · Wohin? = MIȘCARE A→B (verbe: legen/stellen/setzen/hängen/stecken) → Akkusativ.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Articolul corect: Dativ sau Akkusativ?', 'Wo? + Dativ: der/das → DEM · die → DER · die (pl.) → DEN. Wohin? + Akk: der → DEN · die/das → die/das · die (pl.) → die.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += mcBlock('Übung 3 — Verbul potrivit: poziție sau mișcare?', 'Pe Dativ (Wo?): liegt/steht/sitzt/hängt/steckt. Pe Akkusativ (Wohin?): legt/stellt/setzt/hängt/steckt. Forma e identică doar la 2 perechi: hängen + stecken.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += mcBlock('Übung 4 — Asociază perechile de verbe', 'Cele 5 perechi Wechselverben: liegen↔legen · stehen↔stellen · sitzen↔setzen · hängen↔hängen (identic) · stecken↔stecken (identic).', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Wechselpräpositionen + articol corect', 'Cele 9 Wechselpräpositionen: in · an · auf · über · unter · vor · hinter · neben · zwischen. Cu Dativ (Wo?) sau cu Akkusativ (Wohin?) după întrebare.', ex5Data);
        }
        return html;
    }
